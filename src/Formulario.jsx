...
function Formulario({crearColor}){
    ...
    return <form onSubmit={ evento => {
        ...
        if(/^([0-9]{1,3},){2}[0-9]{1,3}/.test(textoInput)){
            ...
            if(valido){
                let [r,g,b] = rgb;
                return fetch("https://api-colores-clase-qy68.onrender.com/nuevo",{
                    method : "POST",
...
setError(true)
        ...
        <input type="text" placeholder="rrr,ggg,bbb"  value={textoInput}
        onChange={evento => setTextoInput(evento.target.value) }/>
        <p className={`error ${error ? "visible" : "" }`}>debe esbribir tres valores entre 0 y 255 separados por comas</p>
